...
def load_and_process_data(uploaded_file):
    ...
    # CORRECTED: This loop is now complete to ensure all categories are populated
...
if uploaded_file:
    ...
    with tab_summary:
        # This tab's code is complete and works
    ...
    with tab_ordering_worksheet:
        ...
        if 'current_worksheet_key' not in st.session_state or st.session_state.current_worksheet_key != worksheet_state_key:
            ...
            editor_df_data = {
                'Item': filtered_df['Item'], 'On Hand': filtered_df['End Inv'],
    ...
    # --- NEW TAB: Sales Mix Analysis ---
    with tab_sales_analysis:
        st.subheader("Sales vs. Actual Usage Variance")
        st.markdown("""
        This tool analyzes the difference between items recorded as **sold** in your POS system and items **used** from your inventory count.
        """)
        
        # --- Configuration is now moved inside the tab ---
        with st.expander("Click to view and edit Sales Mix Configuration"):
            ...
            st.warning("You must map every item you want to analyze to its container size from the list above.")
            ITEM_CONTAINER_MAP = {
                # Draft Beers
                "BEER DFT Alaskan Amber": "Half Barrel Keg", "BEER DFT Blue Moon Belgian White": "Half Barrel Keg",
                "BEER DFT Coors Light": "Half Barrel Keg", "BEER DFT Dos Equis Lager": "Half Barrel Keg",
                # Liquors
                "WHISKEY Buffalo Trace": "Standard Liquor/Wine Bottle", "VODKA Well": "Liter Liquor Bottle",
                # Add all other items you want to analyze here...
            }
        ...
        if sales_mix_file:
            try:
                ...
                def get_inventory_item_and_pour_oz(sales_name, inv_items):
                    sales_name_upper = sales_name.upper()
                    
                    # Exact match first (for bottled beer, wine, liquor)
                    for inv_item in inv_items:
                        if inv_item.upper() == sales_name_upper:
                            if "BEER BTL" in inv_item.upper():
                                return inv_item, 1, 'count' # Return 1 unit, type 'count'
                            elif "WINE" in inv_item.upper():
                                return inv_item, VOLUME_CONFIG["Wine Pour"], 'volume'
                            else:
                                return inv_item, VOLUME_CONFIG["Liquor Pour"], 'volume'

                    # Keyword match for draft beer
                ...
                for _, row in sales_df.iterrows():
                    sales_item_name = row['Item Name']
                    qty_sold = row['Qty']
                    if pd.isna(sales_item_name) or pd.isna(qty_sold): continue
                    
                    inv_item, pour_value, sale_type = get_inventory_item_and_pour_oz(sales_item_name, all_inventory_items)
                ...
                for item, data in total_volume_sold_by_item.items():
                    ...
                    if sale_type == 'volume': # For draft and liquor/wine by the glass
                        ...
                        if not total_container_volume: continue
                        theoretical_usage = total_sold / total_container_volume if total_container_volume > 0 else 0
                        unit = "% of container"
                    else: # For bottled beer (type is 'count')
                ...
                st.error(f"Could not process the Sales Mix file. Please check the file format, column names ('Item Name', 'Qty'), and configuration. Error: {e}")
